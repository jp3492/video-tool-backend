...
const get = async (req, res) => {
  try {
    ...
    const userIds = requests.reduce(
      (res, r) =>
        res.includes(r.from === req.user ? r.to : r.from)
          ? res
          : [...res, r.from === req.user ? r.to : r.from],
      []
    );
    const user = await User.find({ _id: { $in: userIds } });
    const expandedRequests = requests.map(r => ({
      ...r._doc,
      fromMe: r.from === req.user,
      userEmail: user.find(u => u._id === (r.from === req.user ? r.to : r.from))
    }));
